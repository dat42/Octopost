namespace Octopost.Tests
{
    ...
    [TestClass]
    public class VoteTests : OctopostTestBase
    {
        [TestMethod]
        public async Task CheckVote()
        {
            ...
            var id = postCreationService.CreatePost(new CreatePostDto { Text = "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua." });
            ...
            using (var unitOfWork = unitOfWorkFactory.CreateUnitOfWork())
            {
                ...
                Assert.AreEqual(votes.Count, 3);
                Assert.AreEqual(votes[0].State, VoteState.Up);
                Assert.AreEqual(votes[1].State, VoteState.Neutral);
                Assert.AreEqual(votes[2].State, VoteState.Down);
            }
        }
    }
}
